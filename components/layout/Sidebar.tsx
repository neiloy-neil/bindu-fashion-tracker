...
export function Sidebar({ isOpen, setIsOpen }: { isOpen?: boolean, setIsOpen?: (v: boolean) => void }) {
  ...
  const [role, setRole] = useState<'ADMIN' | 'BRANCH' | 'AUDITOR' | 'AREA_MANAGER' | 'HR_ADMIN' | null>(null)
      ...
      <nav className="sidebar-nav" style={{ flex: 1, overflowY: 'auto' }}>
        ...
        {(role === 'ADMIN' || role === 'HR_ADMIN') && (
          <div className="mt-4">
            ...
            <Link href="/hr/employees" onClick={() => setIsOpen?.(false)} className={`nav-item ${pathname.startsWith('/hr/employees') ? 'active' : ''}`}>
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2" /><circle cx="9" cy="7" r="4" /><path d="M23 21v-2a4 4 0 0 0-3-3.87" /><path d="M16 3.13a4 4 0 0 1 0 7.75" /></svg>
              Employees
            </Link>
            <Link href="/hr/salary-processing" onClick={() => setIsOpen?.(false)} className={`nav-item ${pathname.startsWith('/hr/salary-processing') ? 'active' : ''}`}>
            ...
            <Link href="/hr/slips" onClick={() => setIsOpen?.(false)} className={`nav-item ${pathname.startsWith('/hr/slips') ? 'active' : ''}`}>
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" /><polyline points="14 2 14 8 20 8" /><line x1="16" y1="13" x2="8" y2="13" /><line x1="16" y1="17" x2="8" y2="17" /><polyline points="10 9 9 9 8 9" /></svg>
              Salary Slips
            </Link>
            <Link href="/hr/eid" onClick={() => setIsOpen?.(false)} className={`nav-item ${pathname.startsWith('/hr/eid') ? 'active' : ''}`}>
