        ...
        <nav className="flex-1 overflow-y-auto px-2 py-2">
          ...
          {role !== 'HR_ADMIN' && !(role === 'BRANCH' && branchType === 'WHOLESALE') && (<>
            ...
            {items('cashflow', <>
              ...
              {(role === 'BRANCH' || role === 'ACCOUNTS' || role === 'ADMIN' || role === 'SUPER_ADMIN' || role === 'AUDITOR' || role === 'AREA_MANAGER') && item('/transfers/incoming', 'Incoming Transfers', <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><polyline points="20 12 20 22 4 22 4 12"/><rect x="2" y="7" width="20" height="5"/><line x1="12" y1="22" x2="12" y2="7"/><path d="M12 7H7.5a2.5 2.5 0 0 1 0-5C11 2 12 7 12 7z"/><path d="M12 7h4.5a2.5 2.5 0 0 0 0-5C13 2 12 7 12 7z"/></svg>, pendingTransfers)}
              {(role === 'BRANCH' || role === 'ACCOUNTS' || role === 'ADMIN' || role === 'SUPER_ADMIN' || role === 'AUDITOR' || role === 'AREA_MANAGER') && item('/transfers/history', 'Transfer History', <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><path d="M3 3h6l6 9h6"/><path d="M3 21h6l6-9"/></svg>)}
              ...
              {item('/reports/petty-cash', 'Petty Cash Report', <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><rect x="2" y="6" width="20" height="12" rx="2"/><circle cx="12" cy="12" r="3"/><path d="M6 12h.01M18 12h.01"/></svg>)}
            </>)}
          </>)}
          ...
          {(role === 'ADMIN' || role === 'HR_ADMIN' || role === 'BRANCH') && (<>
            ...
            {items('payroll', <>
              ...
              {(role === 'ADMIN' || role === 'HR_ADMIN') && <>
                ...
                {item('/hr/advances', 'Advance Salary', <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>)}
              </>}
            </>)}
          </>)}

          {/* Wholesale — hidden for RETAIL/FACTORY branch users */}
          {(role === 'ADMIN' || role === 'SUPER_ADMIN' || role === 'ACCOUNTS' || role === 'AREA_MANAGER' || role === 'AUDITOR' ||
            (role === 'BRANCH' && branchType !== 'RETAIL' && branchType !== 'FACTORY')) && (<>
          ...
          {(role === 'ADMIN' || role === 'SUPER_ADMIN') && (<>
            ...
            {items('system', <>
              ...
              {item('/admin/locked-months', 'Lock Months', <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>)}
            </>)}
          </>)}
