...
export default function DailyReportTemplate({ entryData }: { entryData: ReportEntryData | null }) {
  ...
  // totalSale = actual revenue only (no opening balance carry-forward)
  const totalSale = incomeItems.reduce((s, i) => s + i.amount, 0)
    + receivedTransfers.reduce((s, t) => s + t.amount, 0)
  // totalIncome = totalSale + openingBalance (used for net cash calculation)
      ...
      <div className="text-center mb-6 border-b border-[var(--border)] pb-6">
        ...
        <div className="grid grid-cols-3 gap-4 mt-2">
          <div className="bg-[var(--success)]/10 border border-[var(--success)]/30 rounded-xl p-4 text-center">
            ...
            <div className="text-2xl font-bold font-mono text-[var(--success)]">৳{formatCurrency(totalSale)}</div>
          </div>
        ...
        <div className="mb-8">
          ...
          <div className="mt-3 flex justify-end">
            <div className="bg-[var(--accent)]/10 border border-[var(--accent)]/30 rounded-lg px-6 py-2 text-right">
              <div className="text-xs text-[var(--text-secondary)] uppercase tracking-wider">Total Sale</div>
              <div className="text-xl font-bold font-mono text-[var(--accent)]">৳{formatCurrency(totalSale)}</div>
            </div>
          </div>
        </div>
      )}
