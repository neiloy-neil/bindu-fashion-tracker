...
export default function NewBuyerModal({ onClose, onCreated }: Props) {
  const [form, setForm] = useState({
    name: '', contactPerson: '', contactNumber: '', secondaryNumber: '',
    email: '', address: '', creditLimit: '', openingBalance: '', branchId: '',
  })
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-[var(--bg-card)] w-full max-w-lg max-h-[90dvh] flex flex-col rounded-xl shadow-xl overflow-hidden border border-[var(--border)]">
        <div className="flex justify-between items-center px-5 py-4 border-b border-[var(--border)]">
          <h2 className="text-base font-semibold text-[var(--text-primary)]">Add Wholesale Buyer</h2>
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input value={form.openingBalance} onChange={set('openingBalance')} type="number" min="0" className={inputCls} placeholder="0" />
            </div>
          </div>

          <div>
            <label className={labelCls}>Linked Branch <span className="text-[var(--text-muted)]">(leave blank for shared)</span></label>
