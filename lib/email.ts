...
export function supportTicketEmail(branchName: string, type: string, description: string) {
  return {
    subject: `[Support] New ${type} request from ${branchName}`,
    html: `
      <p>A new support ticket has been submitted.</p>
      <table style="border-collapse:collapse;width:100%;font-family:sans-serif">
        <tr><td style="padding:8px;font-weight:bold;width:120px">Branch</td><td style="padding:8px">${branchName}</td></tr>
        <tr><td style="padding:8px;font-weight:bold">Type</td><td style="padding:8px">${type}</td></tr>
        <tr><td style="padding:8px;font-weight:bold;vertical-align:top">Description</td><td style="padding:8px">${description}</td></tr>
      </table>
...
export function leaveRequestEmail(employeeName: string, type: string, startDate: string, endDate: string, reason: string | null) {
  return {
    subject: `[Leave] ${employeeName} — ${type} leave request`,
    html: `
      <p>A new leave request has been submitted.</p>
      <table style="border-collapse:collapse;width:100%;font-family:sans-serif">
        <tr><td style="padding:8px;font-weight:bold;width:120px">Employee</td><td style="padding:8px">${employeeName}</td></tr>
        <tr><td style="padding:8px;font-weight:bold">Type</td><td style="padding:8px">${type}</td></tr>
        <tr><td style="padding:8px;font-weight:bold">Dates</td><td style="padding:8px">${startDate} → ${endDate}</td></tr>
        ${reason ? `<tr><td style="padding:8px;font-weight:bold;vertical-align:top">Reason</td><td style="padding:8px">${reason}</td></tr>` : ''}
      </table>
...
export function partyPaymentPendingEmail(branchName: string, partyName: string, amount: number, method: string) {
  return {
    subject: `[Approval] Party payment pending — ${partyName}`,
    html: `
      <p>A party payment submitted by a branch is awaiting your approval.</p>
      <table style="border-collapse:collapse;width:100%;font-family:sans-serif">
        <tr><td style="padding:8px;font-weight:bold;width:120px">Branch</td><td style="padding:8px">${branchName}</td></tr>
        <tr><td style="padding:8px;font-weight:bold">Party</td><td style="padding:8px">${partyName}</td></tr>
        <tr><td style="padding:8px;font-weight:bold">Amount</td><td style="padding:8px">৳${amount.toLocaleString()}</td></tr>
        <tr><td style="padding:8px;font-weight:bold">Method</td><td style="padding:8px">${method}</td></tr>
      </table>
...
export function chequeApprovalEmail(partyName: string, amount: number, withdrawDate: string) {
  return {
    subject: `[Approval] Cheque pending — ${partyName}`,
    html: `
      <p>A cheque payment is pending your approval.</p>
      <table style="border-collapse:collapse;width:100%;font-family:sans-serif">
        <tr><td style="padding:8px;font-weight:bold;width:140px">Party</td><td style="padding:8px">${partyName}</td></tr>
        <tr><td style="padding:8px;font-weight:bold">Amount</td><td style="padding:8px">৳${amount.toLocaleString()}</td></tr>
        <tr><td style="padding:8px;font-weight:bold">Withdraw Date</td><td style="padding:8px">${withdrawDate}</td></tr>
      </table>
      <p style="margin-top:16px"><a href="${process.env.NEXT_PUBLIC_APP_URL}/admin/cheques">Review in Approval Centre →</a></p>
    `,
  }
